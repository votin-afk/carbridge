    ...
    <div className="space-y-6" data-testid="dashboard-catalog">
      ...
      {showFilters && (
        <div className="bg-[#15191E] border border-[#27272A] rounded-lg p-6">
          ...
          {/* Excluded Brands Section - only show after search with results */}
          {hasSearched && brandsInResults.length > 0 && (
            <div className="mb-4">
              <div className="flex items-center justify-between mb-2">
                <Label className="text-slate-400">Исключить марки из результатов</Label>
                <Button
                  variant="ghost"
                  size="sm"
                ...
                >
                  {showExcludeBrands ? 'Скрыть' : `Показать (${brandsInResults.length})`}
                </Button>
              </div>
              ...
              {/* Brand selection for exclusion - from search results */}
              {showExcludeBrands && (
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
                    {brandsInResults.map((brandName) => (
                      ...
                      >
                        <input
                          type="checkbox"
                          checked={excludedBrands.includes(brandName)}
                          onChange={() => toggleExcludeBrand(brandName)}
                          className="w-4 h-4 rounded border-slate-600 bg-slate-800 text-red-500 focus:ring-red-500"
                        />
                        <span className={`text-sm ${excludedBrands.includes(brandName) ? 'text-red-400' : 'text-slate-300'}`}>
                          {brandName}
                        </span>
                      </label>
                    ))}
